...
def get_location_params(json_url):
    response = requests.get(json_url)
    response.raise_for_status()
    return response.json()


def post_location(location_params):
    location, created = Location.objects.get_or_create(title=location_params['title'],
        defaults={
            'short_description': location_params['description_short'],
            'long_description': location_params['description_long'],
            'lng': location_params['coordinates']['lng'],
            'lat': location_params['coordinates']['lat']
        })
    return location.id


def fetch_and_post_image(image_url, location_id, image_number, location_params):
    response = requests.get(image_url)
    response.raise_for_status()

    file_format = image_url.rsplit('.', 1)[-1].lower()
    filename = f'{location_params["title"]}_{image_number}.{file_format}'
...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        for url in options['json_urls']:
            try:
                serialize_location = get_location_params(url)
